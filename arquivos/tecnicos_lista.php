...
// Pega todos os técnicos
$result = mysqli_query($id, "SELECT IDtecnico, nome FROM tecnicos ORDER BY IDtecnico ASC");
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
    <meta charset="UTF-8">
    <title>Lista de Técnicos - Servigera</title>
    <style>
    ...
    <div class="container">
        ...
        <table>
            ...
            <?php while($tecnico = mysqli_fetch_assoc($result)) { 
                // Pega especialidades do técnico
                $resEsp = mysqli_query($id, "SELECT e.nome FROM especialidades e 
                                             JOIN tecnico_especialidade te ON e.IDespecialidade = te.IDespecialidade
                                             WHERE te.IDtecnico = ".$tecnico['IDtecnico']);
                $especialidades = [];
            ...
            <tr>
                ...
                <td><?php echo implode(", ", $especialidades); ?></td>
                <td>
                    <a class="btn" href="editar_tecnico_admin.php?id=<?php echo $tecnico['IDtecnico']; ?>">Editar</a>
                    <a class="btn" href="excluir_tecnico_admin.php?id=<?php echo $tecnico['IDtecnico']; ?>" onclick="return confirm('Tem certeza que deseja excluir este técnico?')">Excluir</a>
                </td>
            </tr>
            <?php } ?>
        </table>
